...
module.exports.register = validate([
  ...
  body('email')
    ...
    .custom(async val => {
      const emailValidate = await prisma.user.findFirst({ where: { email: val } })
      if (emailValidate) {
  ...
  body('phone')
    ...
    .custom(async val => {
      const phoneValidate = await prisma.user.findFirst({ where: { phone: val } })
      if (phoneValidate) {
...
module.exports.update = validate([
  ...
  body('username')
    .custom(async val => {
      const nameValidate = await prisma.user.findFirst({ where: { username: val } })
      if (nameValidate) {
